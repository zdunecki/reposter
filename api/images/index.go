...
func loadFont(dc *gg.Context) {
	fontResp, _ := http.Get("http://localhost:3000/static/Oswald.ttf")
	defer fontResp.Body.Close()

	out, err := os.Create("font.ttf")
	if err != nil {
		panic(err)
	}
	defer out.Close()

	_, err = io.Copy(out, fontResp.Body)
	if err != nil {
		panic(err)
	}

	if err := dc.LoadFontFace("./font.ttf", FontPoints); err != nil {
		panic(err)
	}
}
